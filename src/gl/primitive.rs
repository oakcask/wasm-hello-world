...
pub fn create_primitive<'a, T: VertexArray>(
    ctx: &'a WebGl2RenderingContext,
    data: T,
) -> Result<Primitive<'a, T>, String> {
    let vao = ctx
        .create_vertex_array()
...
impl<'a, T: VertexArray> Primitive<'a, T> {
    pub fn draw(&self, shader: &Shader<'a>) {
        ...
        self.ctx
            .enable_vertex_attrib_array(position_attrib_location);
        self.ctx
            .draw_arrays(self.data.draw_array_mode(), 0, self.data.vertex_count());
